...
def get_tempdir_root():
    local_dir = os.environ.get("LOCAL")
    if local_dir and os.environ.get("SLURM_CLUSTER_NAME"):
        return local_dir

    temp_root = snakemake.config.get("LOCAL_TMPDIR_ROOT", os.path.abspath("local_tmp"))
...
def fastqc():
    ...
    with TemporaryDirectory(dir=get_tempdir_root()) as tempdir:
...
def trimmomatic():
    ...
    with TemporaryDirectory(dir=get_tempdir_root()) as tempdir:
        ...
        log_path = snakemake.log[0] if len(snakemake.log) > 0 else path.join(tempdir, "trimmomatic.log")
