...
def build_feature_matrix(feature_name):
    """Build matrix for one feature type from individual sample files"""
    
    feature_dir = f"{features_folder}/{feature_name}"
    
    print(f"\nBuilding matrix for: {feature_name}")
    ...
    if len(feature_files) == 0:
        print(f"No files found for {feature_name}")
        return
    
    # Load first file to get structure
    first_file = feature_files[0]
    sample_id = os.path.basename(first_file).replace(f"_{feature_name}.csv", "")
    
    df = pd.read_csv(first_file, index_col=0)
    print(f"Feature structure: {df.shape}")
    ...
    if len(df.columns) == 1:
        ...
        for row_idx in df.index:
            for col_name in df.columns:
                ...
                column_names.append(f"{feature_name}_{row_idx}_bin_{clean_col}")
